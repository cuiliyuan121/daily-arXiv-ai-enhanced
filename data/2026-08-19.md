...
一句话总结：本文提出DynaForcing训练框架，解决音频驱动虚拟人生成中蒸馏范式（DMD）的动态坍塌问题，通过混合强制锚定、动力学感知奖励正则化和参考扰动三种策略，在三个层面（数据、损失、条件）恢复时序动态，同时将GPU开销降低一个数量级以上。


<details>
  <summary>详细信息</summary>
研究动机：现有音频驱动虚拟人生成方法采用自强制与DMD蒸馏实现实时流式生成，但存在关键缺陷：学生模型收敛到近静态最优，虽感知质量高但时序动态被严重抑制（即动态坍塌）。这种坍塌对虚拟人尤其有害，因为细微的运动损失会破坏唇形同步和表情。此前该问题尚未被系统性地刻画。

方法：提出DynaForcing框架，包含三个互补策略：1）混合强制（Hybrid Forcing）在数据层面锚定滚动到真实动态以打破反馈回路；2）动力学感知奖励正则化（Dynamics-Aware Reward Regularization）利用DMD的强化学习解释引入显式运动奖励以抵消反向KL偏差；3）参考扰动（Reference Perturbation）扰动参考图像以解耦身份与静态细节，迫使模型依赖音频驱动运动。此外引入计算图剪枝和梯度重放，大幅降低GPU占用。

结果：实验表明DynaForcing将动态退化指标Dyn-Deg从0.31提升至0.73（接近教师水平），唇形同步Sync-C从7.03提升至7.68，同时改善视觉质量，解决了质量-动态折衷问题，且无需早停。

结论：DynaForcing通过多层次互补策略有效解决了自强制蒸馏在音频驱动虚拟人生成中的动态坍塌问题，在保持实时生成能力的同时恢复时序动态并提升视觉质量，且显著降低了计算开销。

原始摘要：Audio-driven avatar generation requires realistic lip-sync, expressive motion, and real-time streaming. Recent work achieves the latter via self-forcing with Distribution Matching Distillation (DMD), but this paradigm suffers from a critical failure that has not been systematically characterized: dynamic collapse, where the student model converges to a near-static optimum with high perceptual quality but severely suppressed temporal dynamics. We trace this to two causes: the reverse KL objective in DMD, which biases toward low-motion modes, and unanchored self-conditioning, which creates a feedback loop that amplifies collapse. This is especially harmful for avatars, where even subtle motion loss breaks lip-sync and expression.
...
### [5] [LinCa: Accelerating Diffusion Models via Learnable Decomposed Feature Caching](https://arxiv.org/abs/2608.17973)
...
一句话总结：AViTS是一个面向动态分辨率扩散Transformer（DiTs）的自适应时空token选择框架，通过融合潜在文本注意力的空间重要性和跨扩散时间步的token级特征变化的时间重要性，实现关键token优先的分辨率细化，在FLUX上实现高达6.34倍加速，在Qwen-Image-Edit和FLUX.1-Kontext-dev上实现近9倍FLOPs减少，与蒸馏模型结合可达14.76倍。


<details>
  <summary>详细信息</summary>
研究动机：动态分辨率采样虽然能通过低分辨率去噪降低早期成本，但在分辨率转换时对所有潜在token进行统一上采样会带来冗余计算，并可能损害细节一致性；现有的部分上采样策略依赖局部潜在结构线索或单步统计，难以联合捕捉token-文本语义相关性和跨扩散步骤的token级表示动态。

方法：AViTS框架通过两条路径建模token重要性：在空间维度使用潜在文本注意力评估空间重要性，在时间维度利用token级特征在扩散时间步间的变化计算时间重要性；然后将两者融合，实现时空重要性感知的选择性上采样，优先细化关键token同时推迟次要token。

结果：在FLUX上实现最高6.34倍加速，在Qwen-Image-Edit和FLUX.1-Kontext-dev上实现近9倍FLOPs减少；与蒸馏、量化、特征缓存等方法正交兼容，与蒸馏模型结合可达14.76倍加速。

结论：AViTS通过时空重要性感知的选择性上采样有效减少了冗余高分辨率计算，在保持生成质量的同时显著改善了质量-效率权衡，且具有良好的方法可组合性。

原始摘要：Diffusion Transformers (DiTs) achieve high-quality generation but are costly due to iterative sampling. Dynamic-resolution sampling reduces early-stage cost by denoising at low resolution; however, uniformly upsampling all latent tokens at resolution transitions incurs redundant computation and may degrade fine-detail consistency. Existing partial upsampling strategies typically rely on local latent structure cues or single-step statistics, making it difficult to jointly capture token-text semantic relevance and token-wise representation dynamics across diffusion steps. We propose AViTS, an adaptive spatiotemporal token selection framework for dynamic-resolution DiTs. AViTS models spatial importance via latent-text attention and temporal importance via token-level feature variation across diffusion timesteps, and fuses them to enable spatiotemporal importance-aware selective upsampling: it prioritizes resolution refinement for critical tokens while deferring less important ones, thereby reducing redundant high-resolution computation and improving the quality-efficiency trade-off. AViTS achieves up to 6.34x on FLUX and nearly 9x FLOPs reduction on Qwen-Image-Edit and FLUX.1-Kontext-dev, orthogonal to distillation, quantization, and feature caching, and reaching 14.76x with distilled models. Code: https://github.com/QHR69/AViTS

</details>


### [7] [EDITBRIDGE: Towards Faithful and Efficient Ultra-High-Resolution Image Editing](https://arxiv.org/abs/2608.18063)
*Jiayi Song,Shijie Huang,Fangtai Wu,Yubo Huang,Zhenxiong Tan,Songhua Liu,Jiaming Liu,Ruihua Huang*

主分类：cs.CV

一句话总结：EditBridge是一种扩散桥接框架，用于高效超高分辨率图像编辑，通过将编辑视为结构化数据到数据的转换（从低分辨率编辑结果到高分辨率结果），并利用先验引导的块级稀疏注意力机制，在4K分辨率下实现高保真编辑，速度提升3.6-8.4倍。


<details>
  <summary>详细信息</summary>
研究动机：现有基于扩散模型的图像编辑受限于二次注意力复杂度和内存需求，无法在1K以上分辨率工作。常见的两阶段方案（低分辨率编辑+独立超分辨率）存在信息发散（幻觉细节与原始高分辨率源冲突）和纹理退化（过度平滑或过度锐化）两大关键问题。

方法：提出扩散桥接框架，将编辑过程建模为从低分辨率编辑结果到高分辨率结果的受控数据到数据转换，明确以原始高分辨率源为条件以保留真实细节。引入先验引导的块级稀疏注意力机制，利用第一阶段编辑中的语义对应关系将跨图像交互限制在空间对齐区域，降低计算开销。

结果：实验表明EditBridge在高达4K分辨率下实现高保真编辑和优越感知质量，在2K分辨率下获得3.6-8.4倍加速，并能在61秒内完成实用化的4K编辑。

结论：EditBridge通过创新性地结合扩散桥接框架与语义先验引导的稀疏注意力，有效解决了超高分辨率编辑中的效率和保真度矛盾，为专业工作流中的高分辨率图像编辑提供了实用解决方案。
