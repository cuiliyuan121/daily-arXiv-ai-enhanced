...
一句话总结：本文提出"Agentic Self-Improvement"框架，将黑盒图像到视频（I2V）生成重构为闭环、目标导向的优化过程。通过两阶段方法——第一阶段利用多模态大语言模型（mLLM）进行迭代提示优化（结合DSG语义查询和CMQ伪影检测），第二阶段使用贝叶斯优化联合优化随机种子和CFG尺度——显著提升生成视频的可控性和可靠性，在人类偏好研究中获胜率高达69%。


<details>
  <summary>详细信息</summary>
研究动机：现有黑盒I2V模型虽有强大的自动内容生成能力，但缺乏细粒度控制和可靠性，提示词或超参数的微小变化会导致输出剧烈波动，迫使专业用户依赖低效的暴力试错过程。

方法：两阶段闭环优化：第一阶段通过mLLM迭代优化输入提示，利用DSG查询确保语义一致性和CMQ进行伪影检测；第二阶段采用贝叶斯优化联合优化随机种子与CFG尺度，由包含新提出的VTA分数在内的质量指标套件引导搜索。

结果：该方法显著优于无引导搜索方法，在人类偏好研究中，智能体方法生成的视频较基线获得高达69%的获胜率。

结论：该工作提供了一种实用且可扩展的方法论，可增强SOTA视频生成模型的可预测性和可控性，推动该领域从投机性探索迈向可靠的生产级工具。
