...
### [1] [Edit-VAR: Taming Visual Autoregressive Model for Precise Video Editing](https://arxiv.org/abs/2609.21268)
*Chongbo Zhao,Jiangming Wang,Xilai Wang,Xinyu Wang,Jingyi Tang,Chunjie Hao,Pengjie Song,Yue Ma*

主分类：cs.CV

一句话总结：提出首个免训练、免反演的文本引导视频编辑框架 Edit-VAR，基于预训练视觉自回归模型实现高效编辑。


<details>
  <summary>详细信息</summary>
研究动机：文本引导视频编辑需在修改目标内容的同时保持未编辑区域外观与时间一致性。现有方法存在不足：免训练方法中，免反演范式编辑强度受限、语义修改不完整；基于反演的范式存在近似误差累积，导致源内容漂移和时间不一致；而基于训练的方法虽控制力强，却需要大量数据和算力。因此亟需一种兼顾编辑质量与效率的新范式。

方法：Edit-VAR 将源视频直接编码为多尺度离散 token，通过概率引导的条件 token 替换实现源保留；采用注意力引导的 token 级与尺度感知调制，在编辑相关位置上选择性地放松源约束；提出尺度解耦生成（以晚期尺度约束释放实现），重建运动一致的细节并减少纹理破碎；并用残差引导的 token 剪枝在高分辨率尺度上利用冗余降低推理成本。

结果：大量实验与盲测用户研究显示，Edit-VAR 在编辑保真度、源内容保留、时间一致性和推理效率方面整体优于现有免训练视频编辑方法。

结论：Edit-VAR 作为首个免训练、免反演的视觉自回归视频编辑框架，通过条件 token 替换、尺度解耦生成与 token 剪枝，在保证编辑效果的同时提升效率，为文本引导视频编辑提供了新思路。
