...
一句话总结：本文提出了VGI-bench，一个包含27个任务、810个实例的视频生成模型视觉推理基准，通过两层级分类体系实现细粒度评估。实验表明当前最强的视频生成模型Seedance 2.0仅达到51.0%的准确率，暴露出视觉推理能力远不可靠的问题。


<details>
  <summary>详细信息</summary>
研究动机：视频生成模型展现出零样本视觉推理的潜力，但缺乏可靠的评估基准。现有评估面临三大挑战：输入需与视频模型的视觉先验对齐、需验证有效的演化过程而非仅最终状态合理、任务难度需保持挑战性又部分可行。

方法：构建VGI-bench基准，包含27个任务和810个实例，采用任务领域和技能标签的两级分类体系进行细粒度评估。评估过程中还分析了输出失败模式、输入条件敏感性、从合成微调的性能迁移边界，以及从内部去噪视角研究模型的自校正能力。

结果：当前生成系统能解决部分视觉推理任务子集，但远不可靠，最强模型Seedance 2.0仅达到51.0%。内部去噪视角分析发现模型存在有限的自校正能力——后续去噪步骤主要精炼早期假设而非纠正推理错误。

结论：VGI-bench揭示了当前视频生成模型视觉推理能力的显著不足，特别是缺乏有效的推理自校正机制。作者希望通过该基准推动下一代视频生成模型的发展。
...
### [3] [Swift-Image: Exploring the Performance Frontier of Compact Unified Image Generation Models](https://arxiv.org/abs/2608.20334)
...
一句话总结：本文提出Swift-Image，一个6B参数的紧凑统一模型，同时支持文生图、单图编辑和多图编辑。通过系统的训练工程（渐进式训练管线、并行专家强化学习、多教师策略蒸馏、Prompt增强器、结构剪枝和少步蒸馏），在仅243K GPU训练小时的有限计算预算下达到了领先的开源模型聚合性能，并成功压缩出3B和加速版本。


<details>
  <summary>详细信息</summary>
研究动机：探索在有限计算预算下，通过系统化的训练工程手段，一个相对较小的视觉生成模型能达到多远的性能极限，即关注"小模型+精细训练"的路径而非单纯堆砌参数和算力。

方法：采用高效的6B单流DiT架构，配合渐进式训练管线（从广泛语义覆盖到高分辨率、强视觉质量和统一生成-编辑监督）；后训练阶段使用并行专家强化学习和多教师在线策略蒸馏来缓解异构目标间的干扰；引入Prompt Enhancer将高层推理与像素级渲染解耦；通过结构剪枝和少步蒸馏生成3B和加速变体。

结果：Swift-Image以仅6B参数和243K GPU训练小时在开源模型的聚合评估中取得领先成绩；压缩后的3B模型几乎没有性能损失；少步蒸馏在显著减少采样步数的同时进一步提升了聚合编辑性能。

结论：系统化的训练工程可以在小模型上实现与大规模模型相当甚至更优的性能，并且模型压缩可以几乎无损地实现。文章还总结了架构、数据课程、后训练、提示增强和模型压缩方面的实用经验教训。
