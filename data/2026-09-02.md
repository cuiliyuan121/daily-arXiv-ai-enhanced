...
### [8] [A Certificate-Producing Cascade for Equational Implication: The SAIR EQT2 Stage 2 Solver](https://arxiv.org/abs/2609.00706)
*Haobo Ma,Wenlin Zhang,Manuel Israel Cázares*

主分类：cs.CL

一句话总结：本文介绍了一个面向SAIR数学蒸馏挑战中方程理论证明任务的单文件求解器，采用最便宜优先的级联架构：否定分支通过结构化代数族系数检验、有限模型搜索及多种无限载体反例模型来生成反驳；肯定分支采用带Knuth-Bendix排序的序单元超归结过程，支持双向demodulation、索引和记忆化替换。所有推导结果在可信基础之外重放验证，不依赖大语言模型。


<details>
  <summary>详细信息</summary>
研究动机：SAIR数学蒸馏挑战要求求解器判定岩浆恒等式之间的蕴含关系，并为判定结果返回能被确定性Lean验证器接受的证书，这对求解器的正确性与可验证性提出了严格要求。

方法：采用最便宜优先的级联策略组织求解器。否定分支由结构化代数族上的系数测试、有界有限模型搜索、显式中心群胚证人和无限载体证人组成；肯定分支采用带Knuth-Bendix排序的证明产出型序单元超归结程序，结合双向demodulation、索引、记忆化替换和任意时间深度增加。

结果：冻结的求解器为单文件Python程序（189,504字节），在官方评审上对六个公共集合的1,889行全部产出被接受的证书，无语言模型调用；对800个Stage 1评估分布问题完全一致，在Marathon清单上无token获得100行接受，在托管playground获得200行接受。

结论：论文明确了这些仅为回归和playground测量结果，并非排行榜成绩，也不构成对隐藏数据集的证据。所有量化声明均关联不可变结果账本，论文不提出完整性或比较优越性主张，核心贡献在于提供了一个完全可验证、不依赖LLM的证明搜索方案。

原始摘要：The SAIR Mathematics Distillation Challenge on Equational Theories asks a solver to classify whether one magma identity implies another and, for either verdict, to return a certificate accepted by a deterministic Lean judge. We present a single-file solver organized as a cheapest-first cascade. Its false branch combines coefficient tests over structured algebra families, bounded finite-model search, an explicit central-groupoid witness, and several infinite-carrier witnesses. Its true branch is a proof-producing ordered unit superposition procedure with Knuth-Bendix ordering, bidirectional demodulation, indexing, memoised substitution, and anytime size deepening. Search results remain outside the trusted base: successful derivations are replayed as small Lean terms, and countermodels are rechecked by the competition judge.
