...
### [19] [WorldCrafter: Consistent Video World Model with Implicit 3D-aware Memory](https://arxiv.org/abs/2609.24984)
...
一句话总结：WorldCrafter 提出一种可相机查询的隐式 3D 感知记忆机制，让视频世界模型在长时程和跨视角下更好地保持历史观测一致性，并支持单图或文本驱动的流式交互式探索。


<details>
  <summary>详细信息</summary>
研究动机：现有视频世界模型虽能进行动态环境的交互式探索，但在长时程与跨视角下难以遵守先前的观测，导致一致性和相机控制表现不足。

方法：核心思想是让请求的视角决定多视角证据如何压缩进视频生成器有限的 token 预算。训练一个记忆编码器和姿态条件化的读出模块，与视频生成器联合训练，在去噪前将历史观测整合为一组固定数量的目标视角专属 token，无需显式的基于深度的对应关系；再结合近期时间上下文与少步蒸馏，实现流式场景探索。

结果：在静态和动态场景实验中，方法在长时程一致性和相机控制精度上取得显著提升，同时在分钟级探索中保持视觉质量。

结论：通过视角驱动的隐式 3D 感知记忆，WorldCrafter 有效提升了视频世界模型在长时程、跨视角下的记忆一致性与相机可控性，并支持从单张图像或文本提示进行流式探索。
