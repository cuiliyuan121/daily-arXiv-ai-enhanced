...
一句话总结：VIPER是一个视觉上下文物理推理框架，通过参考视频提取物理线索，引导图像到视频生成，实现物理行为迁移到新场景，无需复杂提示工程。


<details>
  <summary>详细信息</summary>
研究动机：现有视频生成模型难以控制物理行为（如材料响应、接触交互、形变和运动轨迹），因为文本和图像条件难以详尽描述这些连续的物理线索，而视频能自然演示这些信息。

方法：提出VIPER框架，包含：(1) 使用MLLM从参考视频中提取物理线索；(2) 通过分层训练策略引导预训练图像到视频生成器；(3) 构建VIPER-19K数据集，包含材料、轨迹和物理影响标注。

结果：在未见过的验证集上，VIPER在参考视频物理相似度和人类偏好上优于视频生成和视频即提示基线方法，同时保持有竞争力的视频质量。

结论：VIPER能有效将参考视频中的物理行为迁移到新的目标场景，无需精心设计的提示，解决了视频生成中的物理控制瓶颈问题。
...
### [13] [FilmBench: A Film-Grade Benchmark for Cinematic Video Generation](https://arxiv.org/abs/2607.24241)
