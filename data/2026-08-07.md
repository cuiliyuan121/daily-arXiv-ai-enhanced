...
一句话总结：本文提出Vorch-Omni，一个基于任意条件到任意输出范式的统一多任务音视频合成框架，通过单一流匹配扩散Transformer支持超过10种音视频生成与编辑任务，无需针对特定任务修改架构。


<details>
  <summary>详细信息</summary>
研究动机：现有的生成式视频建模方法往往依赖碎片化的任务特定模型，缺乏一个能统一处理多种音视频生成、参考合成、扩展和编辑任务的通用框架。同时，联合音视频生成因引入跨模态的条件配置和输出目标而更具挑战性，需要区分异质的目标、源和参考信号以减少任务间干扰。

方法：Vorch-Omni采用任意条件到任意输出的统一公式化方案，灵活地将视频和音频信号视为条件输入或生成目标。利用token级条件掩码和任务标识符区分目标、源内容和参考，通过位置类型分离时间上下文与独立条件。采用互补的视觉条件通路：视觉语言模型结合文本指令解释采样帧，视频VAE将条件编码为潜在token提供直接引导。同时构建分布式数据管线来策划时间对齐的音视频片段、生成结构化描述和元数据、平衡异质任务分布。

结果：基于单一流匹配扩散Transformer，无需任务特定的架构修改，Vorch-Omni支持超过10种任务，包括文本到视频、文本到音视频、图像与参考条件生成、时间扩展、音频驱动生成、视频变换和音视频编辑等。

结论：该统一框架为通用音视频生成与操作提供了可扩展的基础设施，有效解决了多任务间的干扰问题，为未来通用多模态生成系统指明了方向。
